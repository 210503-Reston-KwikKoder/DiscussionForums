...
rebuild-db:
    #dotnet ef database drop -c UserTestDBContext --startup-project ../$(solution_root)
	cd ./$(solution_dl) && dotnet ef migrations remove --startup-project ../$(solution_main_BL);
	cd ./$(solution_dl) && dotnet ef migrations add newMigration -c UserTestDBContext --startup-project ../$(solution_main_BL);
